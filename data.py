...
class InputPair:
    ...
    def tokenize_tree(newick_str: str, tokenizer: NewickTokenizer) -> torch.Tensor:
        ...
        """
        tokens = tokenizer.encode(newick_str)
...
class TreeDataset(Dataset):
    ...
    def encode_trees(self, pair: InputPair) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        return gene_trees_tensor, species_tokens
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        gene_trees_tensor, species_tokens = self.cached_encodings[idx]
        ...
        # Copy actual gene tree tokens
        padded_gene_trees[:num_gene_trees] = gene_trees_tensor[:num_gene_trees]

        return padded_gene_trees, species_tokens
...
if __name__ == "__main__":
    ...
    for i in range(min(3, len(dataset_to_inspect))):
        gene_trees_tensor, species_tokens = dataset_to_inspect[i]
        console.print(f"\n[cyan]Item {i}:[/cyan]")
        console.print(f"Gene trees tensor shape: {gene_trees_tensor.shape}")
