...
class GeneTreeDataset:
    ...
    def _sample_unique_quartets(self, rng: Random, n: int) -> list[tuple[int, int, int, int]]:
        """
        ...
        if len(quartets) < n:
            raise RuntimeError(f"Could only generate {len(quartets)} unique quartets after {max_attempts} attempts")
    ...
    def __getitem__(self, idx: int) -> GeneTreeDataPoint:
        ...
        topologies = [single_tree.classify_quartet(quartet) for quartet in selected_quartets]
